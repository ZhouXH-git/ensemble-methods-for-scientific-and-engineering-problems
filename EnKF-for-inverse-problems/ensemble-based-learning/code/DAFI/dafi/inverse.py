...
class REnKF(InverseMethod):
    ...
    * **penalties_python_file** (*string*) -
    ...
    def analysis(self, iteration, state_forecast, state_in_obsspace, obs,
        ...
        hxx = np.dot(hxp, xp.T)
        k2_gain_matrix = \
            coeff * np.dot(kalman_gain_matrix, hxx) - coeff*np.dot(xp, xp.T) # # TODO:
            # coeff * np.dot(kalman_gain_matrix, hxx) - coeff*np.dot(xp, xp.T) # # TODO:
        ...
        for ipenalty in self.penalties:
            ...
            lamda = lamb(iteration) / np.linalg.norm(coeff*np.dot(xp, xp.T)) # TODO:
            # lamda = lamb(iteration) # / np.linalg.norm(coeff*np.dot(xp, xp.T)) # TODO:
            ...
            for isamp in range(self.nsamples):
                ...
                gpw = np.dot(func_gradient(istate, isamp, iteration).T, w_mat)
                gpwg = np.dot(gpw, func_penalty(istate, isamp, iteration))
                # gpw = func_gradient(istate, isamp, iteration).T
                # gpwg = np.dot(gpw, func_penalty(istate, isamp, iteration))
                penalty_mat[:, isamp] += lamda * gpwg
        #         penalty_list.append(func_penalty(istate, isamp, iteration))
        # modelerr = np.linalg.norm(np.array(penalty_list).mean(axis=1))
        # obserr = np.linalg.norm(1. / np.sqrt(np.diag(w_mat)))
        # analysis step
        dx1 = np.dot(kalman_gain_matrix, obs - state_in_obsspace) # TODO:
        dx2 = np.dot(k2_gain_matrix, penalty_mat) # TODO:
        state_analysis = state_forecast + dx1 + dx2 #
        # state_analysis = state_forecast + dx1 + 0.1 * np.linalg.norm(dx1) /  np.linalg.norm(dx2) * dx2

        # test for training with direct data
        # dx1 = 0 # * np.dot(kalman_gain_matrix, obs - state_in_obsspace) # TODO:
        # dx2 = - penalty_mat
        # state_analysis = state_forecast + dx2
        # import pdb; pdb.set_trace()
        # print('norm of dx1 = ', np.linalg.norm(dx1))
        print('norm of dx2 = ', np.linalg.norm(dx2))
        # print('norm of modelerr = ', modelerr)
        # print('norm of obserr = ', obserr)
...
class REnKF_dev(InverseMethod):
    ...
    * **penalties_python_file** (*string*) -
    ...
    def __init__(self, inputs_dafi, inputs):
        """ See :py:meth:`InverseMethod.__init__` for details. """
        super(self.__class__, self).__init__(inputs_dafi, inputs)
        self.name = 'Regularized Ensemble Kalman Filter in development (REnKF)'
        # load penalties
        pfile = inputs['penalties_python_file']
        sys.path.append(os.path.dirname(pfile))
        penalties = getattr(importlib.import_module(
    ...
    def analysis(self, iteration, state_forecast, state_in_obsspace, obs,
        ...
        for ipenalty in self.penalties:
            ...
            for isamp in range(self.nsamples):
                ...
                # import pdb; pdb.set_trace()
                penalty_mat[:, isamp] += lamda * gpwg
                # penalty_mat[:, isamp] += lamda * np.sum(func_gradient(istate, isamp, iteration).T, axis=1) # gpwg

        # analysis step
        # dx1 = np.dot(kalman_gain_matrix, obs - state_in_obsspace) # TODO:
        # dx2 = np.dot(k2_gain_matrix, penalty_mat) # TODO:
        # state_analysis = state_forecast + dx1 + 0.01 * np.linalg.norm(dx1) /  np.linalg.norm(dx2) * dx2

        # test for training with direct data
        # dx1 = 0 # * np.dot(kalman_gain_matrix, obs - state_in_obsspace) # TODO:
        # p = - coeff*np.dot(xp, xp.T)
        # dx2 = p.dot(penalty_mat)
        dx2 = - penalty_mat
        state_analysis = state_forecast + dx2
        # import pdb; pdb.set_trace()
        # print('norm of dx1 = ', np.linalg.norm(dx1))
...
class REnKF_SGD(InverseMethod):
    ...
    * **penalties_python_file** (*string*) -
    ...
    def analysis(self, iteration, state_forecast, state_in_obsspace, obs,
        ...
        # arr = np.arange(self.ncells)
        # np.random.shuffle(arr)
        # index_random = arr.reshape(self.batch, int(self.ncells/self.batch))
        #
        # penalty_mat = np.zeros([len(state_forecast), self.nsamples])
        # for ibatch in range(self.batch):
        #     print('# of batch = ', ibatch)
        #
        #     index = index_random[ibatch, :]
        #     for ipenalty in self.penalties:
        #         w_mat = ipenalty['weight_matrix']
        #         lamb = ipenalty['lambda']
        #         # lamda = lamb(iteration) / np.linalg.norm(coeff*np.dot(xp, xp.T)) # TODO:
        ...
        #             istate = state_forecast[:, isamp]
        #             # gpw = np.dot(func_gradient(istate).T, w_mat)
        #             gpw = np.dot(func_gradient(index, istate, isamp, iteration).T, w_mat)
        #             gpwg = np.dot(gpw, func_penalty(index, istate, isamp, iteration))
        #             # gpw = func_gradient(istate, isamp, iteration).T
        #             # gpwg = np.dot(gpw, func_penalty(istate, isamp, iteration))
        #             penalty_mat[:, isamp] += lamda * gpwg
        #         state_forecast += - penalty_mat
...
class REnKF_inf(InverseMethod):
    """ Implementation of the regularized ensemble Kalman Filter
    (REnKF) with inflation.
    ...
    * **penalties_python_file** (*string*) -
    ...
    def __init__(self, inputs_dafi, inputs):
        """ See :py:meth:`InverseMethod.__init__` for details. """
        super(self.__class__, self).__init__(inputs_dafi, inputs)
        self.name = 'Regularized Ensemble Kalman Filter with inflation (REnKF)'
    ...
    def analysis(self, iteration, state_forecast, state_in_obsspace, obs,
        ...
        ind = Sd_norm.dot(Sd_norm.T)
        # P, D, Q = np.linalg.svd(ind, full_matrices=True)
        # for i in range(len(D)):
        #     if np.sum(D[:i]) / np.sum(D) > 0.99999:
        #         rank = i
        #         print('rank =', rank)
        #         break
        # # rank = 100
        # tsvd = np.matrix(P[:, :rank]) * np.diag(D[:rank]) * np.matrix(
        #     Q[:rank, :])
        # inv = np.linalg.inv(tsvd + self.gamma * np.eye(len(obs_vec)))
        inv = np.linalg.inv(ind + self.gamma * np.eye(len(obs_vec)))

        kalman_gain_matrix = SmSd.dot(inv)

        # import pdb; pdb.set_trace()
        # analysis step
        # dx = np.dot(self.corr * kalman_gain_matrix.A, delta_d)
        ...
        hxx = np.dot(hxp, xp.T)
        k2_gain_matrix = \
            coeff * np.dot(self.corr * kalman_gain_matrix, hxx) - coeff*np.dot(xp, xp.T)
        ...
        # analysis step
        dx1 = np.dot(self.corr * kalman_gain_matrix, obs - state_in_obsspace)
...
# child classes (developing)
class LM_EnRML_new1(InverseMethod):
    """ Implementation of the localized ensemble method (LM_EnRML).
...
# child classes (developing)
class LM_EnRML_new2(InverseMethod):
    """ Implementation of the localized ensemble method (LM_EnRML).
    ...
    def __init__(self, inputs_dafi, inputs):
        """ See  :py:meth:`InverseMethod.__init__` for details. """
        super(self.__class__, self).__init__(inputs_dafi, inputs)
        self.name = 'LM Ensemble Randomized Maximum Likelihood new (LM-EnRML-new)'
        self.inflation_flag = inputs['inflation_flag']
        self.localization_flag = inputs['localization_flag']
        self.alpha = inputs['alpha']
        self.beta = inputs['beta']
    ...
    #     delta_d = np.sqrt(np.linalg.inv(obs_error)).dot(obs - state_in_obsspace)
    #     SmSd = Sm.dot(Sd_norm.T)
    #     # added by xinlei for normalization
    #     SdSd = coeff * hxp.dot(hxp.T)
    #     norm = np.sqrt(np.trace(SdSd)) / self.nsamples
    #     inv = np.linalg.inv(Sd_norm.dot(Sd_norm.T) + self.gamma * norm * np.eye(len(obs_vec)))
    #     kalman_gain_matrix = SmSd.dot(inv)
    #
    #     # analysis step
    #     dx = np.dot(self.corr * kalman_gain_matrix, delta_d)
    ...
    def analysis(self, iteration, state_forecast, state_in_obsspace, obs,
        ...
        conn = _check_condition_number(hpht + obs_error, '(HPHT + R)')
        # SdSd = coeff * hxp.dot(hxp.T)
        norm = np.trace(hpht) / np.trace(obs_error)
        inv = np.linalg.inv(hpht + self.gamma * obs_error)
        kalman_gain_matrix = pht.dot(inv)
        # analysis step
        dx = np.dot(self.corr * kalman_gain_matrix, obs - state_in_obsspace)
        state_analysis = state_forecast + dx
