    ...
    for time in time_array:
        ...
        for iteration in iteration_array:
            ...
            if iteration == 0 and inverse.inflation_flag:
                ...
                inverse.gamma = gamma * inverse.beta
            else:
                inverse.gamma = 1
            ...
            # SGD
            if inverse.name == 'REnKF_SGD':
                ncells = 3000
                batch = 5
                arr = np.arange(ncells)
                np.random.shuffle(arr)
                index_random = arr.reshape(batch, int(ncells/batch))

                penalty_mat = np.zeros([len(state_forecast), inputs_dafi['nsamples']])
                wmat = state_forecast.copy()
                g_truth = np.expand_dims(np.loadtxt('bxy_dns.dat'), axis=-1)

                for ibatch in range(batch):
                    # import pdb; pdb.set_trace()
                    index = index_random[ibatch, :]
                    g, dgdw = model.state_to_grad(wmat)
                    grad = dgdw.reshape(3000, 1, len(state_forecast[:, 0]))
                    wmat -= 0.001 * grad[index, 0, :].T.dot(g[0][index] - g_truth[index])

                    # print('# of batch = ', ibatch)
                print(f'      mini-batch Done ')

                # import pdb; pdb.set_trace()
                print(f'      misfit = ', np.linalg.norm(g[0] - g_truth) )

                state_forecast = wmat.copy()
